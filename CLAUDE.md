...
- **Apple-to-apple is a single flag**: `--apple-to-apple` bundles 8-neighbor smoothing, <1000-spots gene filter, Scanpy-flavor HVG, Reactome+MSigDB pathways, raw pathway scale, last-4-blocks UNI unfreeze, learnable NT-Xent τ, paper gene normalization, section-stratified split, keep-constant-cols, and TabPFN-pure head replacement. Use it for any number that goes into the BIBM 2026 paper. This is the only setting where PEaRL+MLP can be directly compared to the paper's 0.5868 gene PCC / 0.5055 pathway PCC on Breast.
- **Three PEaRL cohorts**: `--cohort {Breast,Skin,Lymph}` selects (default Breast). Per-cohort defaults: `--n-pathways` auto-sets to 775/609/1100; output writes to `reproduction_results/<cohort>/`. The metadata organ string for Lymph is `Lymphoid`, mapped via `COHORT_TO_ORGAN` in `reproduction.py`. HEST-1k currently ships 117 Breast, 80 Skin, and 5 Lymphoid Homo-sapiens sections, so `--n-sections 36` caps at 5 on Lymph automatically.
