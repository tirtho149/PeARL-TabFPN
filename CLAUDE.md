...
- **Apple-to-apple is a single flag**: `--apple-to-apple` bundles 8-neighbor smoothing, <1000-spots gene filter, Scanpy-flavor HVG, Reactome+MSigDB pathways, **min-max [0,1] pathway scale** (matches the paper's MSE≈0.0017/MAE≈0.031), last-4-blocks UNI unfreeze, learnable NT-Xent τ, paper gene normalization, **pooled spot-level 5-fold CV** (`--split spot`), **all spots per section** (no 400 cap), keep-constant-cols, and TabPFN-pure head replacement. Use it for any number that goes into the BIBM 2026 paper. This is the only setting where PEaRL+MLP can be directly compared to the paper's 0.5868 gene PCC / 0.5055 pathway PCC.
- **Which PCC matches the paper**: the paper (and STNet/BLEEP/mclSTExp, the works it benchmarks) reports the **mean per-feature Pearson** — the `PCC_per_dim_mean` field, printed as the `PCC_perdim` row by `print_summary`. The plain `PCC` (`PCC(flat)`) is the global-flatten Pearson and is *not* the paper's definition. Compare `PCC_perdim` to `PAPER_BASELINE_BREAST`.
- **CV protocol — paper vs rigorous**: the PEaRL paper pools all 36 breast sections (13,620 spots) and runs 5-fold CV *over spots* (`--split spot`), which is what reproduces 0.5868/0.5055. `--split section` (GroupKFold, holds out whole sections) is a stricter, leakage-free cross-section-generalization test that is *much* harder (PCC drops 5-8×) and is **not** what the paper reports — keep it for honest-generalization ablations only.
