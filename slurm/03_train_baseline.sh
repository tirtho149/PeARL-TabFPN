...
echo "[train_baseline] cohort=$PEARL_COHORT  node=$(hostname) gpu=$(nvidia-smi -L 2>/dev/null | head -n1 || echo 'no gpu')"
nvidia-smi || true

python scripts/train_baseline.py --apple-to-apple --cohort "$PEARL_COHORT" --n-sections 36 --folds 5
