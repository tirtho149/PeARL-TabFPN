...
| `--pathway-normalization minmax` | Per-pathway min-max to [0,1] — the only scaling consistent with the paper's reported pathway MSE≈0.0017 / MAE≈0.031 (Table 2), and the same scaling the paper uses for genes. Raw ssGSEA leaves pathways on wildly different magnitudes (MSE in the thousands) and lets a few high-variance pathways dominate the global-flatten PCC; z-score inflates MSE ~400×. |
...
| `--split spot` (all spots) | The paper pools all 36 breast sections (13,620 spots) and runs 5-fold CV *over spots* — it does not hold out whole sections. This matches the convention of the works it benchmarks (STNet/BLEEP/mclSTExp) and is the only protocol that reproduces the reported 0.5868 gene / 0.5055 pathway PCC. apple-to-apple also drops the 400-spots-per-section cap so every spot is used. `--split section` (GroupKFold) remains available as a stricter cross-section-generalization test, but it is **much** harder (PCC drops 5-8×) and is *not* the paper's setting. |
